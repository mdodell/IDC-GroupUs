...
//NOTICE: Swap amounts here, swap output in email and in body
include 'totalAmount.php';
$isEarlyBird = false;
$count = ($isEarlyBird ? $totalCount + $_SESSION['count'] : $totalCount);
$var_str = var_export($count, true);
$otherCount = ($isEarlyBird ? $postEarlyCount : $postEarlyCount + $_SESSION['count']);
$var_strOther = var_export($otherCount, true);
$var = "<?php\n\n\$totalCount = $var_str; \n\$postEarlyCount = $var_strOther; \n\n?>";
    ...
    <div class="jumbotron">
        ...
        <div id="center">
            ...
            try {
                ...
                else if($_SESSION['section6ContactEmail']){
                ...
                if($mail->send()){
                    ...
                    echo "<script>document.getElementById('confirmStatus').innerHTML = \"<div class='alert alert-danger'><strong>Registration failed!</strong> Confirmation email failed to send. Please try again or contact Jeanie Morgan (dgjeanie1@gmail.com).</div>\";</script>";
                }
            } catch (Exception $e) {
                echo "<script>document.getElementById('confirmStatus').innerHTML = \"<div class='alert alert-danger'><strong>Registration failed!</strong> Confirmation email failed to send. Please try again or contact Jeanie Morgan (dgjeanie1@gmail.com). Mailer Error: " . $mail->ErrorInfo . "</div>\";</script>";
            }
